...
public class SixButtonGameLayout extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_six_button_game_layout);
    }
    ...
    // NOTE: Blue = 0, Yellow = 1, Red = 2, Purple = 3, Green = 4, Orange = 5
    ...
    public void addNewRandColor() {
        ...
        compColorList.add(rand.nextInt(6));
    ...
    public void checkForNewColor(Random rand, int newColor, int lastColor) {
        if (compColorList.get(newColor) == lastColor) {
            compColorList.set(newColor, rand.nextInt(6));
    ...
    public void gamePlay() throws InterruptedException {
        CountDownTimer timer = new CountDownTimer(compColorList.size() * 750 + 700, 750) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                } else if (compColorList.get(colorIndex) == 3) {
                    display.setBackgroundColor(Color.parseColor("#FFDC00FF"));
                } else if (compColorList.get(colorIndex) == 4) {
                    display.setBackgroundColor(Color.parseColor("#FF32FF00"));
                } else if (compColorList.get(colorIndex) == 5) {
                    display.setBackgroundColor(Color.parseColor("#FFFF7D00"));
                }
                colorIndex++;
            }
    ...
    public void colorButtonValue(View view) throws InterruptedException {
        ...
        }else if (view.getId() == R.id.purpleButton) {
            playerColorList.add(3);    //adds user Pushed button
        }else if (view.getId() == R.id.greenButton) {
            playerColorList.add(4);    //adds user Pushed button
        } else {
            playerColorList.add(5);    //adds user Pushed button
        }
        checkValidColor();
    }
    ...
    public void endGame() {
        AlertDialog.Builder builder = new AlertDialog.Builder(SixButtonGameLayout.this);
        ...
        builder.setPositiveButton(getString(R.string.playAgainText),
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        Intent intent = new Intent(SixButtonGameLayout.this,
